CREATE OR REPLACE FUNCTION public.get_presupuesto_preview(p_evento uuid, p_invitados numeric, p_calendario_espacio_ids uuid[], p_gastronomia_ids uuid[] DEFAULT '{}'::uuid[], p_bodega_ids uuid[] DEFAULT '{}'::uuid[], p_bebida_ids uuid[] DEFAULT '{}'::uuid[], p_tematica_ids uuid[] DEFAULT '{}'::uuid[], p_adicionales_ids uuid[] DEFAULT '{}'::uuid[])
 RETURNS TABLE(calendario_espacio_id uuid, precio_por_invitado numeric)
 LANGUAGE sql
...
AS $function$
    ...
    select
      ...
      case
        when p.invitados is null or p.invitados <= 0 or c.pax_minimo_v is null then 1::numeric
          ...
          with base as (
            select case
              when p.invitados > c.pax_minimo_v then coalesce(p.desc_excedente_tbl, 0)
              when p.invitados < c.pax_minimo_v then coalesce(p.inc_faltante_tbl, 0)
              else 0
            end as base
          ),
          aplicado as (
  ...
  totals as (
    select
      ...
      f.rent_espacio,
      case
        when f.invitados > 0 then round(f.precio_base_evento / f.invitados, 2)
        else 0
      end as espacio_pax,
      coalesce((
        select sum(round(public.menu_precio_base_pax(m.id) * f.factor_menu, 2))
        from public.menu m
        where m.id = any(coalesce(p_gastronomia_ids, '{}'::uuid[]))
      ), 0)::numeric as gastro_total,
      coalesce((
        select sum(round(public.bodega_precio_base_pax(b.id) * f.factor_menu, 2))
        from public.bodega b
        where b.id = any(coalesce(p_bodega_ids, '{}'::uuid[]))
      ), 0)::numeric as bodega_total,
      coalesce((
        select sum(round(public.bebida_precio_base_pax(be.id) * f.factor_menu, 2))
